		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if(ts.p.cellEdit === true) {
				if(ts.p.multiselect && scb && cSel){
					...
					ri = ptr[0].rowIndex;
					try {$(ts).jqGrid("editCell",ri,ci,false);} catch (_) {}
				}
				return;
			}
			if (!cSel) {
...
"use strict";
$.jgrid.extend({
	editCell : function (iRow,iCol, ed, val, navigate){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				// azg: added .select() as well
				window.setTimeout(function () { $(elc).focus();$(elc).select(); },1);
				
				// azg: set on type
				if (val != undefined) window.setTimeout(function () { $(elc).val(val);},1);
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && !e.shiftKey) {
						...
						// azg: move to next cell
						if ($t.p.cellMove == "next")
							$($t).jqGrid("nextCell",iRow,iCol);
						// azg: move to down cell
						else if ($t.p.cellMove == "down")
						{
							var ev = jQuery.Event("keydown");
							ev.which = 40; // # Some key code value
							ev.keyCode = 40; // # Some key code value
							$("#"+$t.p.id + "_kn").trigger(ev);
						}
					...
					// if moved with arrow keys, save and advance
					...
					if (e.keyCode === 37 || e.keyCode === 38 || e.keyCode === 39 || e.keyCode === 40)  
					{
						...
						{
							$($t).jqGrid("saveCell",iRow,iCol);
							// azg: move to next navigation cell
							var ev = jQuery.Event("keydown");
							ev.which = e.keyCode; // # Some key code value
							ev.keyCode = e.keyCode; // # Some key code value
							$("#"+$t.p.id + "_kn").trigger(ev);
						}
					}
				...
				cc.addClass("edit-cell ui-state-highlight");
				// $($t.rows[iRow]).addClass("selected-row ui-state-hover"); // azg removed highlight class for row/cell
					...
					if(cv[0] === true) {
						...
						if ($t.p.cellsubmit === 'remote') {
							if ($t.p.cellurl) {
								...
								$($t).jqGrid("progressBar", {method:"show", loadtype : 'disable', htmlcontent: $.jgrid.defaults.savetext || "Saving..." });
								$t.grid.hDiv.loading = true;
								
								// azg: assuming success for fast response
	...
	nextCell : function (iRow,iCol) {
		return this.each(function (){
			...
			if(nCol !== false) {
				...
				// try to find next row editable cell
	...
	prevCell : function (iRow,iCol) {
		return this.each(function (){
			...
			if(nCol !== false) {
				...
				// try to find last row editable cell
				for (i=$t.p.colModel.length; i>=0; i--) {
					if ( $t.p.colModel[i].editable ===true && $t.p.colModel[i].hidden ===false) {
						nCol = i; break;
					}
				}
		...
		return this.each(function () {
			...
			.keydown(function (e){
				...
				switch (kdir) {
					...
					default :
						// azg: if display chars value (http://unixpapa.com/js/key.html)
						if ( (kdir >= 65 && kdir <= 90) || (kdir>=48 && kdir<=57) 
								|| ($.inArray(kdir,[59,61,188,173,190,191,192,219,220,221,222,109,111,78,46]) !== -1)
								|| (kdir>=96 && kdir<=107)  )
						{
							// azg: paste
							if (kdir == 86 && e.ctrlKey)
							{
								// $($t).jqGrid("editRow",$t.rows[$t.p.iRow].id,true);
								// $(".ui-jqgrid .editable").css({padding: '0px',border: '0px',margin: '0px',background: 'inherit'});
								// setTimeout(function(){
									
									// var ev = jQuery.Event("paste");
									// ev.keyCode = 86;
									// ev.ctrlKey = true;
									// $(document).trigger(ev);
								// },200);
							...
							{
								e.key = '';
								$($t).jqGrid("editCell",$t.p.iRow,$t.p.iCol,true,e.key,true);
								setTimeout(function(){ $($t).jqGrid("saveCell",$t.p.iRow,$t.p.iCol); },1);
								return true;
							}
							
							// disable all ctrl combinations
							if (e.ctrlKey)
								return true;
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			ruleFieldSelect.bind('change',function() {
				...
				$(".input-elm",trpar).bind('change',function( e ) {
					...
					// azg: for multi-selectbox, set comma sep values (02-11-2017)
					if(cm.stype === "select" && cm.searchoptions!==undefined && cm.searchoptions.multiple===true) {
	...
	columnChooser : function(opts) {
        ...
        opts = $.extend({
            ...
            "apply_perm" : function() {
				...
				// azg: optimized grouping header redraw
				var t_groupHeader = self[0].p.groupHeader;
				...
				// azg: optimized grouping header redraw
				jQuery("tr.ui-jqgrid-labels", self[0].grid.hDiv).not("tr.ui-jqgrid-labels:last").remove();
				
				if(t_groupHeader)
					self.jqGrid("setGroupHeaders", t_groupHeader);
